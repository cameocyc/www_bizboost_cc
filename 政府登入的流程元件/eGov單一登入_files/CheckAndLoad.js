	...
	*/
	...
	function CheckBrowserAndLoad(){  //判斷Browser後運行元件
		...
		if (osName == "Windows") {	//Windows, Linux, Mac
			...
			alert("請使用Windows/Mac作業系統，並執行ServiSign簽章元件");
	...
	ServiSignErrorCode = 61001;
			if (LoadState != undefined) {//ServiSign載入成功
				alert(LoadObjectName + "載入成功");
				GetErrorMessage(ServiSignErrorCode);
			}
			else {  //ServiSign載入失敗
				alert("載入失敗，錯誤代碼:" + GetErrorMessage(ServiSignErrorCode));
			}
	}
	...
	/*function CheckServiSignInstalled(){  //檢查是否安裝ServiSign 1/0
		...
		if (getCookie("hasServiSign") == ""){
			...
			if (checkLoad != undefined) {
					...
					if(DownloadURL == ""){
						alert("1請安裝ServiSign簽章元件才可進行交易，如果已安裝元件，請重新啟動瀏覽器再試一次");
					}else{
			...
			checkLoad = undefined;
			if(DownloadURL == ""){
				alert("2請安裝ServiSign簽章元件才可進行交易，如果已安裝元件，請重新啟動瀏覽器再試一次");
			}else{
				//window.location = "" + DownloadURL;
				alert("2");
			}
			...
			if(DownloadURL == ""){
				alert("安裝資訊異常，如果已安裝元件，請重新啟動瀏覽器再試一次");
			}else{
				//window.location = "" + DownloadURL;
				alert("3");
			}
