...
function getHCAAPISVIAdapterObj() {
	...
	function verifyURL(input){
		...
		//if(input.indexOf('http://localhost:') != 0 && input.indexOf('https://127.0.0.1:') != 0){
		//	return ""
  //      }
        if (input.indexOf('https://127.0.0.1:') != 0) {
            return ""
        }
	...
	function getServiSignObj() {
		...
		var Url_Part1_DNS = 'http://localhost:'
		...
		var ServiSignObj = 
		{
			...
			TryPort : function() {
                ...
                for (var i = 0; i < sUrlVariables.length; i++) {
					...
					if (parameterName[0].toLowerCase() === "level") {
                        hcalevel = parameterName[1];
                    }
                }
	...
	var ServiSignInterface = 
	{
		...
		ATL_InitSession : function(ulModuleHandle, iFlags, userPin){
            ...
            //console.log("ATL_InitSession 612")
            //console.log(DataObj)
			return ServiSignObj.Send(DataObj)
		},
		...
		ATL_GetErrorCode : function(){
            ...
            //console.log("ATL_GetErrorCode 1089:",DataObj)
			return ServiSignObj.Send(DataObj)
		},
        ...
        ATL_GetString: function (tobesign) {
            var DataObj = new Object()
            DataObj.functionname = "ATL_GetString"
            DataObj.String = Base64Encode(tobesign)
